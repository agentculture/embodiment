...
- `examples/scopebench_live.py` (task `t11`): the pre-registered ScopeBench series, dialled. A live strategist is a `PlannerFn`, so `subordinate.execute` grades it through byte-identical machinery to the scripted controls; the system message is `embodiment.scope.SCOPE_AUTHORITY` verbatim; roles resolve by name from `/capabilities` through `examples/scope/seats.py`. It introduces no clock — every dial rides `worker_seam`'s already-derived constants, and `t11` added the two `(role, budget)` pairs it fronts to `tests/test_timeout_bounds.py::CLOCKS`. Kept out of `examples/scope/` so the pre-registration's hermetic-folder claim stays true
- Stage-1 live results (`docs/live-test-results/scopebench.md`): **`INCONCLUSIVE` under the committed rule, and every condition Stage 1 could evaluate HELD.** 216 live calls, 36 committed episodes per arm. Condition 2 — the one proving the upper-level *decision* contributed — held on **6 of 6** families at sign margins +3/+5/+4/+4/+3/+5; the verified trap was taken **1 of 36** against the baseline stand-in's 36; **0 authority violations**; non-intervention came in **28.7 points better** than the baseline, not merely inside tolerance. Conditions 1 and 5 are `ABSENT` (both read from Stage 2), which is what makes the verdict inconclusive. Two negatives published with the same prominence: `A3` is still behind the scripted `revising` control on **4 of 6** families, and the layer control `A2` was **not measured** — 32 of 36 cells `void-protocol`. Four pre-registration amendments recorded in §15
...
- `SCOPE_AUTHORITY` is **not** patched here and the gap is filed instead: `ScopeRegister` refuses a directive whose `scope_id` is already in the chain, and the shipped system message never says the id must be new. `t11` measured the cost — 47 of the worker seat's 93 offered directives and 9 of the cortex's 87 refused as duplicates — and left the text alone, because changing the protocol mid-series would have measured the fix rather than the architecture
...
- The SonarCloud backlog is triaged to zero untriaged (task `t16`): 45 `python:S9073` composite assertions split across 13 test files so a failing test names *which* conjunct failed — 44 the server reported plus one it could not yet see, because a branch push produces no analysis — and 4 `python:S9083` empty-parenthesis decorators fixed. The three that remain are accepted with rationale on the issue and in `docs/sonar-dispositions.md`: `python:S107` on `loop.run` (17 params, the package's headline public entry, ~90 call sites) and on `continuity.recall` (14 params, a documented one-for-one mirror of `eidetic recall`'s flags and the literal `RecallFn` seam), and `python:S1172` on `_PullSeam.drain`'s `step_count` (unread, but named by keyword at the only call site and mandated by the `MuseSeam` protocol). Zero production behaviour changed; the suite is 6469 passed either side
- A test that was **not** testing what it claimed, found by the first real SonarCloud analysis of the scope lane (PR #76, `python:S5863`, the only `BUG` in the batch and the sole cause of `new_reliability_rating: 3`). `test_solving_is_deterministic` asserted `solve(e).to_dict() == solve(e).to_dict()`, but `oracle._solver_for` caches `_Solver` instances in a module-level dict keyed by the episode's content hash — so the second call reused a solver whose memo tables were already full and re-derived nothing. The test measured a populated cache read twice. A public `oracle.reset_solver_cache()` makes the cache visible rather than a private trap, the test now clears it between two independent solves, and the `_Solver` docstring's false claim *"Constructed per solve, never shared"* is corrected. The rewrite was verified to *strengthen*, not silence: non-determinism injected into the memoised search is **passed** by the old assertion shape and **failed** by the new one
- The other nine PR #76 findings: 4 `python:S5778` exception tests where a second call inside the `raises` block could satisfy it (most consequentially `test_an_unknown_keyword_is_refused_by_runs_own_signature`, where a `TypeError` from a fixture would have left it green while `run_scoped` accepted the bogus keyword), 1 `python:S5958` `pytest.raises(Exception)` narrowed to `FrozenInstanceError`, 1 `python:S3415` assertion whose local named `actual` held the *reference* value, and 1 of the 2 `python:S3776` — `scope._forbidden_key` split into a shape dispatch plus two named traversals, proved behaviour-preserving by differentially fuzzing the pre-refactor implementation over 60,008 payloads (0 mismatches). Two accepted with rationale: `python:S8495` on `_as_tuple` (a variable-length sequence coercion, not a fixed-arity record — its `list`-returning twin in `contract.py` has never raised it) and `python:S3776` on `_first_object` (a lexer whose only available extraction cannot be trusted from its name, and whose `raw_decode` alternative merges two distinct `DEGRADED_MALFORMED` reasons with the suite staying green)
