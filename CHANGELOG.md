...
task `t21`) publishes latency as measured, including if it is bad, in
`docs/live-test-results/2026-09-22-t21-acceptance.md`. This entry lists what
shipped and, with the same weight, what did not.

### Added

- **The daemon** (`daemon/app.py`, `daemon/lifecycle.py`; `embodiment start`
  / `status` / `stop`). Hot mic on `start` through the reSpeaker XVF3800
  (`pw-record`/`pw-play` over PipeWire, `arecord`/`aplay` as fallback —
  deviation `d4`; mute enforced in the capture path before encode). An
  ears-only session on lobes' `/v1/realtime` at the array's native 16 kHz
  (`realtime/wire.py`, `realtime/client.py`): the daemon runs the turn and
  never sends `response.create`. Every reply is generated by the `senses`
  role and spoken through `POST /v1/audio/speech` (`voice.py`, sentence by
  sentence, a bounded body read, redirects refused). Barge-in is the daemon's:
  speech during playback stops its own speaker. Hardware echo cancellation is
  declared (`aec_mode=aec`) and unmeasured until `t21`. `start` returns once
  the daemon reports itself running; `stop` is bounded and records an
  escalation to `SIGKILL`; `status` never starts anything and says that a
  live process is not evidence that anything was heard.
- **One spoken turn on the loop** (`turn.py`, `tools.py`, `session.py`):
  `loop.run` with an **empty** tool registry, never silent, never raises; an
  explicit-ask detector (Hebrew/English), a turn queue, supersede on barge-in,
  a summary on close.
- **Memory that is private by default** (`memory.py`): `RoomMemory` over
  continuity, pinned to a private store under the state directory,
  deadline-bounded. Two things are remembered — an explicit spoken ask and one
  session-end summary. Recalled memory enters the prompt at exactly one place,
  quoted, attributed and fenced. Deviation `d5`: eidetic's keyword tokeniser is
  ASCII-only, so a Hebrew recall the index cannot tokenise records
  `app-recall-lexical-blind` and retries in exact mode within the recall
  deadline; the recall mode actually in effect is reported in `status` and on
  the dashboard.
- **The HTTP surface** (`http/guard.py`, `http/server.py`): loopback by
  default, or a tailnet bind with `--http-bind` + `--bind-public` +
  `--allowed-host`; the install secret (`<state dir>/install-secret`, 0600)
  on every guarded request and on the SSE stream, because the stream carries
  the transcript; a Host/Origin allow-list; the Cloudflare Access assertion
  seam. `status` reports `http.secure_context_required` when a plain-http
  non-localhost bind cannot vouch for the cookie.
- **The dashboard** (`web/`, React/Vite/TS, shipped inside the wheel by a
  build hook — `tests/test_wheel_ships_the_dashboard.py`): a live waveform of
  what is being played (min/max envelope, hi-DPI), the transcript, every
  degradation, the remote-viewer count and the recall mode. It streams events
  with `fetch` and sends the secret in the `Authorization` header (the cookie
  path fails on plain http off localhost). `BrowserEar` is built and tested
  and **not wired** in v1.
- **The inbound realtime endpoint** (`audio/remote.py`): the lobes wire
  inbound, first-message auth in constant time, one peer, a bounded handshake
  reclaim. A seam for a browser ear or a robot relay; **not composed by the
  daemon in v1**.
- **`embodiment tunnel`**: prints the `cultureflare remote-login setup` and
  `cloudflared tunnel run` commands and runs neither; no `--apply` exists.
  Arguments are charset-checked at parse time and every printed token is
  `shlex.quote`-d.
- **State, safety and observability** (`daemon/state.py`, `safe_reason.py`,
  `bus.py`, `audio/features.py`): a 0700 state dir, bounded logs, a
  crash-durable degradation ledger, private per-session transcript logs
  (0600); degradation reasons that never carry speech, a secret or a raw id;
  an in-process bus with redaction at publish and optional MQTT; a streaming
  feature extractor with no IO. Raw audio is never written to disk.
- **The gateway key comes from `grant`**:
  `grant run --inject EMBODIMENT_GATEWAY_KEY=LOBES_GATEWAY_API_KEY -- embodiment start`.
  No env-file fallback; the key never reaches a browser, an asset, an event, a
  log or a `status` field.
- `websockets>=15` joins the approved dependency set (deviation `d2`;
  `tests/test_zero_deps.py`). `sounddevice` was approved and then withdrawn
  (`d4`).
- `tests/test_release_docs.py`: the software-presence boundary, the
  `reachy-mini-cli` `agent embody` name collision and the not-yet list are
  pinned against the README and the `explain` root; the README's status must
  say unmeasured, one rig, and cite `t21`'s result file.

### Not in this release

- **Tools.** The registry is empty by design; Gwen cannot act on anything.
- **Vision.** The session is audio-only; `senses` does not advertise image
  understanding on this rig.
- **A face.** The waveform is the centrepiece.
- **Phone voice.** The phone is control and text.
- **A robot relay.** `reachy-mini-cli` owns the body and its own
  `agent embody` layer; this package is software presence, not a body.
- **Cloudflare Access verification.** The guard requires the assertion on a
  public Host and refuses every one (`http-access-verifier-missing`) until an
  RS256 dependency is approved. The dashboard is not reachable
  through a public hostname today.
- **Semantic recall.** The embedder is down on the rig; recall is lexical.
- **Reconnection.** A dropped realtime session leaves the daemon running
  deaf, recorded, until it is restarted.
- **A second language, a second ear, a second machine.** Hebrew only; one
  active ear; the session never crosses machines (the gateway's `stt` lane
  must be local).
- **A measured value claim.** None is made.

### Known, not yet measured

- The gateway's segmenter can drop the first word after a silence (3 of ~8
  synthetic utterances in the integration runs; the operator's ~30 live turns
  did not show it). Pause, then speak. `t21` measures it.
- Whether the rig's seam reports `completion_tokens` (without it every turn
  carries `turn-truncation-undetectable`), whether `is_speakable` matches what
  the synthesiser voices, and whether Gwen ever transcribes her own voice —
  all `t21`'s.

### Changed

- `README.md`, `CLAUDE.md` and the `explain` root rewritten for the daemon
  release: status (built, one rig, unmeasured, `t21`'s result file), how to
  run, the boundary, the not-yet list. `CLAUDE.md` keeps the three lessons
  and the archive pointer.

### Fixed

- `.markdownlint-cli2.yaml` now ignores `web/node_modules/**`. The config's
  `node_modules/**` matches only the repo root, and the t19 build hook runs
  `npm ci` on any install where `web/dist` is missing — a fresh CI checkout
  after `uv sync` — so the lint job would have failed on 4144 errors in
  third-party READMEs before it ever reached ours.
...
## [0.7.0] - 2026-07-25 (continued)

This block shipped in 0.7.0 (#13, the same commit as the heading above) and
carried the heading `[Unreleased]` until the first daemon release, when a
second `[Unreleased]` made the stale one a lint error.
