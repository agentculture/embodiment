...
class TestALiveWorkspaceAtCloseIsFindable:
    ...
    def test_the_context_manager_closes_it_when_the_body_raises(self) -> None:
        ...
        # the assertion — so it is extracted into a local rather than hoisted
        # out, leaving ``pytest.raises`` wrapping exactly one call
        # (python:S5778). ``match=`` stays load-bearing either way: only the
        # host's own bug can satisfy it, never a stray ValueError from
        # ``__enter__`` or ``execute``.
        def body_raises_inside_the_workspace() -> None:
            with workspace:
                workspace.execute(WORKSPACE_TOOL_NAME, {"command": ["true"]})
                raise ValueError("the host's own bug")

        with pytest.raises(ValueError, match="the host's own bug"):
            body_raises_inside_the_workspace()
