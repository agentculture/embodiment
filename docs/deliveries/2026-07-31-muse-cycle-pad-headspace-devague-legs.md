...
| `t13` (`d17`) *(added after this summary was first written)* | **Found by the reviewer on PR #38, and it was a real hole.** `d2` recorded the *condition* for importing headspace (module-scope import only if the Python API is declared public, otherwise subprocess the CLI) but nothing recorded how it **resolved** — so the ledger never named the dependency, its version floor, or the justification, even though `CHANGELOG.md`, `CLAUDE.md` and `tests/test_zero_deps.py` all carried it. `d17` closes that: the floor is `>=0.11` because 0.11.0 is the release that declared `headspace.api` public under headspace's own semver, and `pyproject.toml` now points at the ledger so a floor nobody chose cannot survive review | `acceptable` |
