...
- acceptance:
  ...
  - the scope modules join the curated public surface in the same `__init__.py` pass: embodiment.scope, embodiment.`scoped_run` and embodiment.`strategist_runner` are exported so examples can import them directly. THREE agents independently hit tests/`test_demo_greenhouse.py`::TestPublicApiOnly and each shipped a documented workaround — t9 mirrored the Scope\* dataclasses and `FORBIDDEN_DIRECTIVE_KEYS` in examples/scope/subordinate.py behind 6 pinning tests, t7 made `governed_by`() return a plain dict instead of constructing ScopeGovernor (examples/scope/seats.py), and t8 duck-typed Directive/Responsibility/Outcome/Snapshot/LaneDegradation stand-ins in examples/scope/`greenhouse_scope.py`. Retire all three workarounds in the same change and keep their pinning tests as real-import assertions
