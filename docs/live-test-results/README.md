...
| [scopebench.md](scopebench.md) | the strategic-scope architecture, Stage 1: does a strategist *above* the actor loop decide better than the actor loop alone, with a deterministic perfect subordinate executing — arms `A0`/`A2`/`A3`, 36 committed episodes per arm, 216 live calls | **`INCONCLUSIVE`, and every condition Stage 1 could evaluate HELD.** Condition 2 — the one that proves the *decision* contributed — held on **6 of 6** families at sign margins +3/+5/+4/+4/+3/+5; trap taken **1 of 36** against the baseline's 36; **0 authority violations**; non-intervention **28.7 points better**, not merely not-worse. But `A3` is still behind the scripted `revising` control on **4 of 6** families, and the layer control `A2` was **not measured** — 32 of 36 cells `void-protocol`. Root cause is a defect in the shipped package: `ScopeRegister` refuses a duplicate `scope_id` and `SCOPE_AUTHORITY` **never says the id must be new** |
| [scopebench-preregistration.md](scopebench-preregistration.md) | the episode schema, committed seeds, exact oracle, four arms, four disjoint record axes and the seven-condition verdict rule | committed **before the first measured dial**; four amendments recorded in §15, one of them a harness wording that was corrected from a pilot and both arms restarted from scratch |
| [scopebench-config-preregistration.md](scopebench-config-preregistration.md) | ScopeBench **cycle 2**: the config-change arm `A4`, its layer control `A5`, the advisory comparator `A3`, the lane as a second declared arm field, the seven conditions **plus a ratchet condition**, the per-change-type three-way ladder, per-arm protocol floors and an admission pilot | committed **before any cycle-2 dial**; declares in advance that the config lane has **no Stage-1 cell** (condition 2 is `N/A` for it, permanently) and that **3 of the 7 change types are out of this bench's reach**. Names risk `r1` (cross-type composition) as partially in scope with the bound written down, and records the senses checkpoint swap as **pending, not in force** |
