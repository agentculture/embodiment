...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope embodiment
...
scope:** the default personal scope is **private** (`--scope embodiment
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `embodiment`). Private records are served only to a recall in the same
